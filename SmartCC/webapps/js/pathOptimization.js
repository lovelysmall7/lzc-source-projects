...
//计算最优路径
function getOptimizatePath(data) {
    var AStar = function() {
                ...
                if (node.I != GID) { //若不是终点
                    ...
                    for (i = 0, j = nodes.length; i < j; i++) {
                        ...
                        if (!(tempnode = maps[id])) { //如果没有查过
                            ...
                            //计算代价值，估价值，总值
                            tempnode.F = (tempnode.G = node.G + euclidean(tempnode, node)) + (tempnode.H = euclidean(tempnode, goal));
    ...
    wc = new AStar; //创建ASatr对象
    wc.loadMap(data.mapData); //载入地图
    SetSortPoint(data.pList);
   // alert(pList.length);
    pathCaculate();
