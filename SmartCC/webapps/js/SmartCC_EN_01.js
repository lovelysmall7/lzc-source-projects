...
function submitScheme(){
	...
	var taskRows = $('#taskTable').datagrid('getSelections');
...
function searchTaskInfo(){
	...
	var searchBatchNo = $("#searchBatchNo").combobox('getValue');
	var searchType = $("#searchType").combobox('getValue');
	$.post( "/SmartCC/smartCCIndex", 
	{ 
		searchBatchNo: searchBatchNo,
		searchType:searchType,
		method:'searchTaskInfo'
	},
	function (data){
		if(data.code == 0){
			$('#taskTable').datagrid('loadData', data.data);
...
function deleteDivision(index){
	...
	var rows= $('#divideResult').datagrid('getSelections');
	var dataJson = JSON.stringify(rows);
	$.post( "/SmartCC/smartCCIndex", { dataJson:dataJson,method:'deleteDivision'},function (data){
			if(data.code == 0){
				$('#taskTable').datagrid('loadData', data.data);
				$('#divideResult').datagrid('loadData', data.data1);
				$('#tt').datagrid('loadData', data.data2);
...
function taskTypeFormatter(val,row){
	if(val == 0){
		return "<span style=''>Normal</span>" 
	}
	if(val == 1){
		return "<span style=''>Emergency</span>" 
	}
	if(val == 2){
		return "<span style=''>Recheck</span>" 
	}
}
function downLoad(){
	var searchWorker = $("#cc").combobox('getValue');
