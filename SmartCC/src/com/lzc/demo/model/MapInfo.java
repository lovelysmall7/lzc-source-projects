...
public class MapInfo {
	...
	private ArrayList<ArrayList<Integer>> mapData = null;
	private AStarMapInfo astartMapInfo = null;
	private String em = "WINDOWS";
	//private String em = "LINUX";
	public void setMapInfo(ArrayList<ArrayList<HashMap<String,Object>>> mapinfo){
		this.mapinfo = mapinfo;
	}
	/**
	 * 构建完成地图信息 包含工位名称 柱子号 坐标  等完整信息
	 * @return
	 */
	public ArrayList<ArrayList<HashMap<String,Object>>> getMapInfo(){
		if(mapinfo == null){
			...
			if("WINDOWS".equals(em)){
				excelPath = excelPath.substring(1,excelPath.length()); 
			}
			System.out.println("map path: " + excelPath);
	...
	public AStarMapInfo getAstartMapInfo(){
		if(astartMapInfo == null){
			astartMapInfo = new AStarMapInfo();
			astartMapInfo.buildMap(getMapData());
		}
		return astartMapInfo;
	}
	private ArrayList<ArrayList<Integer>> buildMapData(){
		ArrayList<ArrayList<HashMap<String,Object>>> mapinfo = getMapInfo();
		//构建地图信息=--------------------
