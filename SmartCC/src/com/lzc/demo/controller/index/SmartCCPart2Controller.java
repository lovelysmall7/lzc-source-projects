...
public class SmartCCPart2Controller extends BaseServlet {
	...
	public void ExportTaskInfo(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		...
		//build the title of excel that will be exported
		HashMap<String,Object> title1 = new HashMap<String,Object>();
		title1.put("label", "Part");
		title1.put("property", "componentName");
		HashMap<String,Object> title2 = new HashMap<String,Object>();
		title2.put("label", "Part station");
		title2.put("property", "stationName");
		HashMap<String,Object> title3 = new HashMap<String,Object>();
		title3.put("label", "Category");
		title3.put("property", "type");
		HashMap<String,Object> title4 = new HashMap<String,Object>();
		title4.put("label", "Staff No.");
		title4.put("property", "workerId");
		HashMap<String,Object> title5 = new HashMap<String,Object>();
		title5.put("label", "Date");
		title5.put("property", "createTimeStr");
		HashMap<String,Object> title6 = new HashMap<String,Object>();
		title6.put("label", "status");
		title6.put("property", "checkResult");
		...
		titleArray.add(title6);
		String titleJson = JSONArray.fromObject(titleArray).toString();
		List<HashMap<String,Object>> dataList = tDao.ExportTaskInfo(t);
		for(int i = 0;i < dataList.size();i++){
			HashMap<String,Object> ttt = dataList.get(i);
			Date d = (Date) ttt.get("createTime");
			if(d != null){
				ttt.put("createTimeStr", sdf.format(d));
			}
			Integer checkResult1 = (Integer) ttt.get("checkResult");
			if(checkResult1 == null){
				ttt.put("checkResult", "-");
			}
			else if(checkResult1 == 1){
				ttt.put("checkResult","NOK");
			}
			else if(checkResult1 == 0){
				ttt.put("checkResult","OK");
			}
			else{
				ttt.put("checkResult", "-");
			}
		}
		ExcelUtil.exportExcel(request, response, "inspection_result.xls", titleJson, dataList);
	}
	/**
	 * 条件查询更新下面的任务列表
	 * @param request
	...
	private void buildBarDataByTime(String startTime, String endTime, HashMap<String, Object> result)
		...
		result.put("code", 0);
		// ------这些是柱状图的数据支撑-------------
		ArrayList<String> dateArray1 = new ArrayList<String>();
		for(int m = 0;m < dateArray.size();m++){
			String dateStr = dateArray.get(m);
			if(!StringUtils.isEmpty(dateStr)){
				dateStr = dateStr.replace("-","");
				dateArray1.add(dateStr);
			}
		}
		result.put("xAxis", dateArray1);
